...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'join-room': {
        const { roomCode, name, invisible } = msg;
        const room = rooms.get(roomCode);
        if (!room) { ws.send(JSON.stringify({ type: 'error', message: 'Room not found' })); return; }

        peer.roomCode = roomCode;
        peer.name = name || 'Guest';
        peer.role = room.participants.size === 0 ? 'host' : 'participant';
        ...
        // Notify others (if invisible, only host/mods get notified)
        if (invisible) {
          room.participants.forEach((p, pid) => {
            if (pid === id) return;
            if (p.role === 'host' || p.role === 'moderator') {
              p.ws.send(JSON.stringify({ type: 'peer-joined', peer: { id, name: peer.name, role: peer.role, invisible: true }, isInvisible: true }));
            }
          });
        } else {
          broadcast(roomCode, { type: 'peer-joined', peer: { id, name: peer.name, role: peer.role, invisible: false } }, id);
        }
        break;
      }
